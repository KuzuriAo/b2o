...
#### Running this as an unattended pipeline

Everything above also works as real infrastructure, not just a personal folder. Point `--watch` (or a cron-scheduled `convert`) at wherever new `.3mf` files actually land: a marketplace's incoming-uploads folder, a print farm's order queue, a shared drive a community bot drops files into, and it keeps producing Snapmaker-ready output with no one watching it.

```bash
b2o convert /var/uploads/incoming \
  --watch \
  --out-dir /var/uploads/converted \
  --archive-dir /var/uploads/incoming/done \
  --log-file /var/log/b2o.log \
  --quiet
```
...
- **All three supported source slicers work interchangeably.** A pipeline doesn't need to know or care whether a given upload came from Bambu Studio, Creality Print, or Anycubic Slicer Next; vendor detection and profile matching both happen per file, automatically.
- **A permanently bad file is skipped on every future run, not retried forever.** A corrupt or unparseable file gets flagged once and left alone from then on. A transient failure (a network blip, a momentary rate limit) is retried automatically instead. That distinction is what keeps one bad upload from silently blocking every file behind it in the queue.
- **`--archive-dir` means nothing is ever rescanned twice.** Once a file converts, it moves out of the watched folder entirely, so there's no state to lose track of and no risk of double-processing after a restart.
- **`--log-file` plus `--quiet` gives a clean audit trail**, with nothing on stdout to fill up a process supervisor's own logs.

For an org/vendor-scale quota to match this kind of continuous volume, see "API keys & quotas" below.
